...
class RevisionMiddleware(object):
    ...
    def process_request(self, request):
        ...
        request.META['%s-%s' % (REVISION_MIDDLEWARE_FLAG, self)] = True
    ...
    def _close_revision(self, request):
        """Closes the revision."""
        if request.META.get('%s-%s' % (REVISION_MIDDLEWARE_FLAG, self), False):
            del request.META['%s-%s' % (REVISION_MIDDLEWARE_FLAG, self)]
